...
--
--     Postgres REJECTS it:
--       ERROR: column "s.name" must appear in the GROUP BY clause or be used
--              in an aggregate function
--     There is one MAX but many names, so which name should come back is
--     undefined. SQLite happens to accept it and silently picks a row.
...
--      Duplicate names in the alias list are accepted at DEFINITION time here;
--      Postgres only complains when you REFERENCE the ambiguous name:
--        ERROR: column reference "colxxx" is ambiguous
--      (MySQL rejects the definition outright, SQLite silently resolves to the
--      first column, DuckDB renames the second to colXXX_1.)
-- WITH cteName (colXXX, colXXX) AS (SELECT 1, 2) SELECT colXXX + colXXX FROM cteName;
